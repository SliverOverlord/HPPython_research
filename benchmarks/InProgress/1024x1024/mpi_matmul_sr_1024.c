...
int main(int argc, char* argv[])
{
    ...
    int col_split = 1024 / (world_size - 1);
    int chunkSize = col_split * 1024;

    static double mat[1048576];
    static double mat2[1048576];

    //static double local_mat[row_split][1000];
    static double *local_mat;
    static double *local_mat2;
    static double *local_output_mat;
    static double gathered_output_mat[1048576];

    int offset = 1048576 / (world_size - 1);
    double startTime, endTime;

    FILE *fp, *fp2;
    if (world_rank == MASTER)
    {
            fp = fopen("1024x1024_matrix.txt", "r");
            fp2 = fopen("1024x1024_matrix.txt", "r");
    ...
    local_mat2 = (double *) malloc(sizeof(double) * 1048576);
    local_output_mat = (double *) malloc(sizeof(double) * 1048576);
    ...
    if (world_rank == MASTER)
    {
        ...
        for (int process = 1; process < world_size; process++)
        {
            ...
            MPI_Send(&mat2, 1048576, MPI_DOUBLE, process, 2, MPI_COMM_WORLD);
        }
        //MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (world_rank != MASTER)
    {
        ...
        //{
            ...
            MPI_Recv(local_mat2, 1048576, MPI_DOUBLE, MASTER, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            //printf("val: %lf \n", local_mat[chunkSize-1]);
            fflush(stdout);
        //}
        //MPI_Barrier(MPI_COMM_WORLD);
        //printf("Recv has been completed.\n");
        mat_mult_double_1d(local_mat, col_split, 1024,
                           local_mat2, 1024, 1024,
                           local_output_mat, col_split, 1024);

        //print_1d_mat(local_output_mat, col_split, 1024);
    ...
    if (world_rank == MASTER)
    {
        ...
        endTime = MPI_Wtime();
        //MPI_Barrier(MPI_COMM_WORLD);
        print_1d_mat(gathered_output_mat, 1024, 1024);
